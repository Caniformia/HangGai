...
struct QuestionListTab: View {
    ...
    @Binding var selectedQuestionList: String
    ...
    @ViewBuilder func descriptionText(of text: String, identifier: String) -> some View {
        Text(text)
            .opacity(selectedQuestionList == identifier ? 1 : 0)
            .font(.body)
            .padding(selectedQuestionList == identifier ? 10 : 0)
            .lineSpacing(2.5)
            .overlay(RoundedRectangle(cornerRadius: 10)
                        .stroke(colorScheme == .dark ? Color.white : .black)
                        .foregroundColor(colorScheme == .light ? Color.white : .black)
            )
            .frame(maxWidth: selectedQuestionList == identifier ? nil : 0, maxHeight: selectedQuestionList == identifier ? nil : 0)
            .padding([.top, .leading], selectedQuestionList == identifier ? 5 : 0)
            .padding(.trailing, selectedQuestionList == identifier ? 15 : 0)
    }
                    ...
                    HStack(alignment: .top) {
                        ForEach(questionListTable, id: \.self.id) { (id, name, key, description) in
                            HStack(alignment: .top) {
                                Button(action: {
                                    ...
                                    VStack(alignment: .center, spacing: 0) {
                                        RoundedRectangle(cornerRadius: 6)
                                            ...
                                            .frame(width: selectedQuestionList == key ? 50 : 40, height: 3)
                                            .opacity(0.5)
                                            .padding(.bottom, 3)
                                            ...
                                            VStack(alignment: .center, spacing: 0) {
                                                ForEach(Array(name), id: \.self) { char in
                                                    AnimatableBoldText(
                                                        ...
                                                        identifier: key,
                                                        selectedIdentifier: $selectedQuestionList,
                                ...
                                descriptionText(of: description, identifier: key)
                            }.opacity(selectedQuestionList == key ? 1 : ((questionManager.getQuestionListCount(identifier: key) == 0) ? 0.1 : 1))
                        }
                    }
                }
            } else {
                            ...
                            VStack(alignment: .leading) {
                                Button(action: {
                                    ...
                                    HStack(alignment: .center) {
                                        RoundedRectangle(cornerRadius: 6)
                                            .foregroundColor(Color(hex: 0x91989F))
                                            .frame(width: 3, height: selectedQuestionList == key ? 50 : 40)
                                            .opacity(0.5)
                                            .padding(.trailing, 3)
...
struct QuestionListTab_Previews: PreviewProvider {
    static var previews: some View {
        QuestionListTab(selectedQuestionList: .constant("Meta"), showQuestionListTab: .constant(true))
